...
function getAdapter({ deploymentStrategy, responseMode, }) {
...
export default function createIntegration(entrypointParameters = {}) {
    ...
    if (integrationConfig.deploymentStrategy !== "regional" &&
        integrationConfig.responseMode === "stream") {
        throw new Error(`Deployment strategy ${integrationConfig.deploymentStrategy} does not support streaming responses. Use 'buffer' response mode.`);
    }
    return {
        name: PACKAGE_NAME,
        hooks: {
            "astro:config:setup": ({ config, updateConfig }) => {
                if (integrationConfig.deploymentStrategy !== "static" &&
                    ...
                    if (typeof entrypointParameters.deploymentStrategy === "undefined") {
                        ...
                        console.log(`[astro-sst] Overriding output to 'server' to support ${integrationConfig.deploymentStrategy} deployment.`);
                        updateConfig({
                            output: "server",
                        });
                    }
                ...
                if (integrationConfig.deploymentStrategy === "static" &&
                    config.output !== "static") {
                    console.log(`[astro-sst] Overriding output to 'static' to support ${integrationConfig.deploymentStrategy} deployment.`);
                    updateConfig({
                        output: "static",
                    });
                }
